...
class PrecomputeEmbeddings:
    ...
    def preprocess_titles(self, books):
        """
        책 제목 데이터를 텍스트 형태로 변환
        """
    ...
    def save_title_embeddings_and_similarities(
        self,
        db: Session,
        title_embeddings_file="title_embeddings.npy",
        title_similarities_file="title_similarities.npy",
    ):
        """
        책 제목 데이터를 기반으로 제목 임베딩 벡터와 제목 간 유사도 행렬 저장
        """
        all_books = db.query(Book).all()
        ...
        all_titles = self.preprocess_titles(all_books)
        if not all_titles:
            print("책 제목 데이터가 비어 있습니다. 임베딩 생성을 중단합니다.")
            return

        # 제목 임베딩 계산
        title_embeddings = self.get_embeddings(all_titles)

        # 제목 간 코사인 유사도 계산 (함수 없이 내부에서 수행)
        norm_embeddings = title_embeddings / np.linalg.norm(
            title_embeddings, axis=1, keepdims=True
        )
        title_similarities = np.dot(norm_embeddings, norm_embeddings.T)

        # 저장 디렉토리 확인 및 생성
        os.makedirs(os.path.dirname(title_embeddings_file), exist_ok=True)

        # 제목 임베딩과 유사도 저장
        np.save(title_embeddings_file, title_embeddings)
        np.save(title_similarities_file, title_similarities)

        print("제목 임베딩과 유사도 저장 완료")
